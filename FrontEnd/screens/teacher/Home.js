...
export default function Home() {
  ...
  const loadQuizes = async (username) => {
    try {
      const res = await axios.post(api + "/api/Quiz/get-teacher-quizes", { username })
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={{ flexDirection: 'row', marginRight: wp('4'), alignItems: 'center' }}>
          <TouchableOpacity onPress={() => navigation.replace('Quiz_Create')}>
            <Ionicons name="add" size={wp('6%')} color="#efededff" style={styles.headerIcon} />
          </TouchableOpacity>
          <TouchableOpacity>
